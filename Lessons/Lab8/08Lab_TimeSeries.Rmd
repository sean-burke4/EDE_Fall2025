...
Months <- as.data.frame(seq.Date(from=as.Date("1906/01/01"),
...
BR_Raints <- ts(BR_Complete$Rainfall,
            start = c(f_year, f_month),
            frequency = 12)

BR_Tempts <- ts(BR_Complete$Temperature_C,
            start = c(f_year, f_month),
            frequency = 12)
...
BR_Rain_Decomp <-stl(BR_Raints,s.window = "periodic")
plot(BR_Rain_Decomp)

BR_Temp_Decomp <-stl(BR_Tempts,s.window = "periodic")
plot(BR_Temp_Decomp)
