...
NTL_data <- read.csv(
  file=here('Data', 'Raw', 'NTL-LITER_Lake_ChemistryPhyscs_Raw.csv'),
  stringsAsFactors = TRUE
)
# 2.
library(tidyverse);library(here)
folder <- here('Data', 'Raw')
files <- dir(folder, pattern = 'EPAair_O3', full.name = TRUE)
EPAData <- files%>%
  map(read_csv)%>%
  bind_rows()
```
...
usgs_flow_data$month <- factor(usgs_flow_data$month)
