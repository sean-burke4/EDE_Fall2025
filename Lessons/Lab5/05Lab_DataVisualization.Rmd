...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    label="paste('Mean is ', mu,',', R^{2}, '=0.6')",
    parse = TRUE
    )
...
Daily_Surface_Temp_1999_plot <- PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
  ggplot(aes(
      x=daynum,
      y=temperature_C,
...
Daily_Temp_1999_plot <- PeterPaul.chem.nutrients %>%
  ggplot(aes(
      x=sampledate,
      y=temperature_C,
      color=depth)
